...
client = MongoClient(MONGODB_URI)           # pas de connexion ici
...
def ensure_indexes():
    """Crée les index sans bloquer le boot si la DB n'est pas reachable."""
    try:
        ...
        # On log juste en stdout pour Heroku, mais on ne casse pas l'app
        print("WARN: ensure_indexes failed:", e)

# --- CORS ---
# CORS_ORIGINS="https://ton-site.netlify.app,http://localhost:3000"
...
# Autoriser toutes les previews Netlify (optionnel)
...
# Préflight OPTIONS catch-all (ne pas ajouter d’headers à la main)
...
def update_reminder(rid):
    data = request.get_json() or {}
    fields = {k: data[k] for k in ["title", "description", "priority", "status", "assigned_to"] if k in data}
    if "due_date" in data:
